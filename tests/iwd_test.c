/*
 * hxfs_iwd unit tests — synthetic ZIP only (no copyrighted archives).
...
static void test_cod_paths(void) {
    const char iwi[] = "IWi";
    const char gsc[] = "main(){}";
    TestFile files[2];
    size_t len = 0;
    ...
    files[0].name = "images\\loadscreen.iwi";
    files[0].data = iwi;
    files[0].size = 3;
    files[1].name = "weapons/mp/ak47";
    files[1].data = gsc;
    files[1].size = (unsigned int)strlen(gsc);
    files[1].deflate = 1;
    ...
    free(bytes);
    CHECK(z != NULL);
    CHECK(zip_contains(z, "images/loadscreen.iwi"));
    CHECK(zip_contains(z, "WEAPONS/MP/AK47"));
    CHECK(zip_read(z, "weapons/mp/ak47", &buf, &sz));
    CHECK(sz == (unsigned int)strlen(gsc) && buf && memcmp(buf, gsc, sz) == 0);
    free(buf);
    zip_close(z);
}
...
static void test_plugin(void) {
    ...
    unsigned char* buf = NULL;
    unsigned int sz = 0;
    const char* path_a = "hxfs_iwd_test_a.iwd";

    memset(a, 0, sizeof(a));
    a[0].name = "sound/null.wav";
    a[0].data = hello;
    a[0].size = 3;
    ...
    CHECK(plug != NULL);
    CHECK(plug->format_name && strcmp(plug->format_name, "iwd") == 0);
    CHECK(plug->open(plug, path_a) == 1);
    CHECK(plug->read(plug, "SOUND/NULL.WAV", &buf, &sz) == 1);
